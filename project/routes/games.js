...
router.post("/addGame", async (req, res, next) => {
  try {
    ...
    const hometeamId = hometeams.data.data[0].id
    ...
    if (awayteams.data.data.leauge!=null && awayteams.data.data.leauge.data.id!=271)
      throw { status: 409, message: "Team is not in the league" };
      const awayteamId = awayteams.data.data[0].id
    ...
    await DButils.execQuery(
      `INSERT INTO dbo.Games (gamedate, gametime, hometeam, hometeamId, awayteam, awayteamId, feild, referee) VALUES ('${req.body.game_date}','${req.body.game_time}','${req.body.hometeam}',${hometeamId},'${req.body.awayteam}',${awayteamId},'${req.body.feild}','${req.body.referee}')`
    );
